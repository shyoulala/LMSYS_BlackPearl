...
class qWenSFTDataset(Dataset):
    ...
    def __init__(self, dataset, tokenizer, max_prompt_len, max_completion_len) -> None:
        ...
        for _, data in tqdm(dataset.iterrows()):
            text = data['text']
            input_ids = tokenizer(text.replace(tokenizer.eos_token, "<end>"))['input_ids'][:1100]
            input_ids.append(tokenizer.eos_token_id)
            labels = data['label']
            if "winner_model_a_2" not in data:
                self.grads.append([0] * 6)
            else:
                self.grads.append(
                    [data['winner_model_a'], data['winner_model_b'], data['winner_tie'], data['winner_model_a_2'],
                     data['winner_model_b_2'], data['winner_tie_2']])
...
def main():
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            logits = outputs.logits
            grads = batch['grads']
            grads1 = batch['grads'][:, :3]
            grads2 = batch['grads'][:, 3:]
            labels = batch['labels']
